$(function() {
	...
	$('#create_sub').click(function() {
		...
		var radio = $(".exist_app_li").find('input[type=radio]:checked');
		if ($(radio[0]).val() == 1) {
...
//显示编辑页面
function show_edit_page(name, rootpath, notes, cweb, username, pwd, exist_app,is_send_app_msg) {
	$(".second_bg").show();
	$(".second_box.new_point").show();
	...
	is_exist_app(is_send_app_msg,exist_app, username, pwd);
	$('#must_fix').hide();
	text_value(name, rootpath, notes, cweb);
}

//是否存在app账号
function is_exist_app(is_send_app_msg,exist_app, username, pwd) {
	var is_send_app = $('.is_send_app_msg');
	if (is_send_app_msg) {
		$(is_send_app[1]).removeAttr("checked");
		$(is_send_app[0]).attr("checked", true);
	} else {
		$(is_send_app[0]).removeAttr("checked");
		$(is_send_app[1]).attr("checked", true);
	}
	var radio = $('.exist_app');
	if (exist_app) {
